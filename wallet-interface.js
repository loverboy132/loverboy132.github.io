...
import { ngnToPoints, pointsToNgn } from "./supabase-client.js";
            ...
            <div class="modal-body">
                <form id="withdraw-form">
                    <div class="form-group">
                        ...
                        <input type="number" id="withdraw-amount" name="amount" min="3000" step="150" required>
                        <small class="form-help">
                            Minimum withdrawal: ₦3,000 (20 points). Withdrawals are converted to points at ₦150/point and may be rounded down to the nearest ₦150.
                        </small>
                    </div>
...
async function handleWithdrawSubmit(e) {
    ...
    const formData = new FormData(e.target);
    const amountNgn = parseFloat(formData.get('amount'));
    const bankDetails = {
    ...
    // Validate amount in NGN before proceeding
    if (!Number.isFinite(amountNgn) || amountNgn <= 0) {
        showNotification('Please enter a valid amount in Naira (₦).', 'error');
        return;
    }

    // Convert NGN to points and normalize NGN to the actual processed amount
    const points = ngnToPoints(amountNgn);
    const normalizedNgn = pointsToNgn(points);

    // Business rule validations
    if (amountNgn < 3000) {
        showNotification('Minimum withdrawal amount is ₦3,000 (20 points).', 'error');
        return;
    }

    if (points === 0 || points < 20) {
        showNotification('Minimum withdrawal is 20 points (₦3,000).', 'error');
        return;
    }

    // Inform user when their entered amount is rounded down due to point conversion
    if (normalizedNgn !== amountNgn) {
        const requestedAmountText = formatCurrency(amountNgn);
        const processedAmountText = formatCurrency(normalizedNgn);
        const pointsText = formatPoints(points);
        showNotification(
            `Your ${requestedAmountText} request converts to ${pointsText}, so the processed amount will be ${processedAmountText} (rounded to ₦150 multiples). Continue.`,
            'info'
        );
    }
    ...
    try {
        ...
        // Quick pre-check using NGN balance
        if (availableNgn < amountNgn) {
            showNotification(
                `Insufficient balance. You are trying to withdraw ${formatCurrency(amountNgn)}, but your available balance is ${formatCurrency(availableNgn)} (${formatPoints(availablePoints)}).`,
                'error'
            );
            return;
        }
        ...
        submitBtn.innerHTML = '<i class="fas fa-paper-plane fa-spin"></i> Submitting...';
        await createWithdrawalRequest(points, bankDetails);
