...
// --- Shared signup guards + cooldown helpers ---
const SIGNUP_COOLDOWN_MS = 2 * 60 * 1000; // 2 minutes
let memberSignupInFlight = false;
let apprenticeSignupInFlight = false;

function normEmail(email) {
    return (email || "").trim().toLowerCase();
}

function cooldownKey(role, email) {
    return `signup_cooldown_${role}_${normEmail(email)}`;
}

function getCooldownRemainingMs(role, email) {
    const until = Number(localStorage.getItem(cooldownKey(role, email)) || 0);
    return Math.max(0, until - Date.now());
}

function setSignupCooldown(role, email, ms = SIGNUP_COOLDOWN_MS) {
    localStorage.setItem(cooldownKey(role, email), String(Date.now() + ms));
}
...
// --- Subscription helpers (single source of truth = DB timestamps) ---
...
/**
 * Compute subscription activity from DB fields.
 * - subscription_status must be 'active'
 * - subscription_expires_at must be in the future
 */
export function getSubscriptionActivity(profile) {
    const expiresAt = safeParseDate(profile?.subscription_expires_at);
    const startedAt = safeParseDate(profile?.subscription_started_at);
    const isActive =
        profile?.subscription_status === "active" &&
        !!expiresAt &&
        expiresAt > new Date();
    return { isActive, expiresAt, startedAt };
}

/**
 * Returns the plan key to use for gating.
 * If the user is expired/inactive, this returns 'free' even if subscription_plan is 'creative'.
 */
export function getEffectiveSubscriptionPlanKey(profile) {
    const rawPlan = (profile?.subscription_plan || "free").toLowerCase();
    if (rawPlan === "free") return "free";
    const { isActive } = getSubscriptionActivity(profile);
    return isActive ? rawPlan : "free";
}

async function refreshSubscriptionStatusIfPossible(userId) {
    if (!userId) return;
    try {
        const { error } = await supabase.rpc("refresh_subscription_status", {
            p_user_id: userId,
        });
        if (error) throw error;
    } catch (error) {
        // 42883 = undefined_function (RPC not deployed yet). Fail open.
        const msg = error?.message || "";
        if (error?.code === "42883" || msg.includes("does not exist")) {
            return;
        }
        console.warn("refresh_subscription_status RPC failed (ignored):", error);
    }
}
...
/** Resend signup confirmation email. Use after signup success. */
...
/** Show a Resend verification email button after the success message element. */
function showResendButton(successElementId, email) {
    const successEl = document.getElementById(successElementId);
    if (!successEl || !email) return;
    const container = successEl.parentElement;
    if (!container) return;
    let wrap = container.querySelector("[data-signup-resend-wrap]");
    if (wrap) {
        wrap.remove();
    }
    wrap = document.createElement("div");
    wrap.setAttribute("data-signup-resend-wrap", "1");
    wrap.className = "mt-2";
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "text-sm text-blue-600 hover:text-blue-800 underline focus:outline-none";
    ...
    btn.addEventListener("click", async () => {
        ...
        try {
            ...
            const errEl = document.getElementById(successElementId.replace("-success", "-error"));
            if (errEl) {
                errEl.textContent = err?.message || "Failed to resend. Try again.";
                errEl.classList.remove("hidden-view", "hidden");
            }
        }
    });
...
export async function generateReferralCode(userId) {
    try {
        // Only MEMBERS can have referral codes
        ...
        // Fail closed (DB validates codes server-side)
        return null;
    }
}
...
// Strict member-only helper for dashboard referral code usage
export async function generateOrGetReferralCode(userId) {
    try {
        const { data: profile, error: profileError } = await supabase
            .from("profiles")
            .select("role")
        ...
        console.error("Error in generateOrGetReferralCode:", error);
        return null;
    }
}
...
export async function handleLogin(email, password) {
    ...
    try {
        ...
        // Server-side: auto-downgrade expired subscriptions on login
        await refreshSubscriptionStatusIfPossible(session.user.id);

        // Check user's role to determine redirect destination (profile created by trigger; retry once if delayed)
        ...
        if (profileResult.error && (profileResult.error.code === 'PGRST116' || profileResult.error.message?.includes('not found'))) {
        ...
        const { data: profile, error: loginProfileError } = profileResult;

        if (loginProfileError) {
            console.warn("Could not fetch user profile (trigger may be delayed). Redirecting to dashboard.");
...
/**
 * Request a password reset email. User will receive a link to reset-password.html.
 * In Supabase Dashboard → Auth → URL Configuration, add Redirect URL:
 *   http://127.0.0.1:5500/reset-password.html (and your production domain reset-password.html).
 */
...
export async function handleMemberSignup(
    ...
    const signupBtn =
        document.querySelector("#member-signup-btn") ||
        document.querySelector("#signup-member-form button[type=\"submit\"]");

    if (memberSignupInFlight) {
        return;
    }

    const remaining = getCooldownRemainingMs("member", normalizedEmail);
    if (remaining > 0) {
        showError(
            "signup-member-error",
    ...
    memberSignupInFlight = true;
    setButtonBusy(signupBtn, true, "Sending email...");
    ...
    try {
        ...
        const fullNameValue = (name && String(name).trim()) || normalizedEmail.split("@")[0] || "Member";
        const emailRedirectTo =
            typeof window !== "undefined"
                ? `${window.location.origin}/login-supabase.html`
                : undefined;

        console.debug("[signup] member signUp called for", normalizedEmail);
        ...
        if (authError) {
            ...
            if (isRateLimitError(authError)) {
                setSignupCooldown("member", normalizedEmail, SIGNUP_COOLDOWN_MS);
                showError(
                    "signup-member-error",
                    "Email sending limit reached. Wait 2 minutes, then try again. If you already signed up, use Sign In or Forgot Password."
                );
                return;
            }
            ...
            if (isAlreadyRegisteredError(authError)) {
                showError(
                    "signup-member-error",
                    "This email already has an account. Please Sign In or use Forgot Password."
                );
                return;
            }
        ...
        if (!authData?.user) {
            showError("signup-member-error", "User creation failed. Please try again.");
            return;
        }

        updateMessageElement(
            "signup-member-success",
            "Check your email to verify your account before logging in."
        );
        showResendButton("signup-member-success", normalizedEmail);

        // Optional short cooldown after success to prevent hammering
        setSignupCooldown("member", normalizedEmail, 30 * 1000);
...
export async function handleApprenticeSignup(
    ...
    const normalizedEmail = normEmail(email);
    const signupBtn =
        document.querySelector("#apprentice-signup-btn") ||
        document.querySelector("#signup-apprentice-form button[type=\"submit\"]");

    if (apprenticeSignupInFlight) {
        return;
    }

    const remaining = getCooldownRemainingMs("apprentice", normalizedEmail);
    if (remaining > 0) {
        showError(
            "signup-apprentice-error",
            `Too many attempts. Try again in ${Math.ceil(remaining / 1000)}s.`
        );
        return;
    }

    apprenticeSignupInFlight = true;
    setButtonBusy(signupBtn, true, "Sending email...");
    ...
    try {
        ...
        const fullNameValue = (name && String(name).trim()) || normalizedEmail.split("@")[0] || "Apprentice";
        const emailRedirectTo =
            typeof window !== "undefined"
                ? `${window.location.origin}/login-supabase.html`
                : undefined;

        console.debug("[signup] apprentice signUp called for", normalizedEmail);
        ...
        if (authError) {
            ...
            if (isRateLimitError(authError)) {
                setSignupCooldown("apprentice", normalizedEmail, SIGNUP_COOLDOWN_MS);
                showError(
                    "signup-apprentice-error",
                    "Email sending limit reached. Wait 2 minutes, then try again. If you already signed up, use Sign In or Forgot Password."
                );
        ...
        if (!authData?.user) {
            showError("signup-apprentice-error", "User creation failed. Please try again.");
            return;
        }

        updateMessageElement(
            "signup-apprentice-success",
            "Check your email to verify your account before logging in."
        );
        showResendButton("signup-apprentice-success", normalizedEmail);

        // Optional short cooldown after success to prevent hammering
        setSignupCooldown("apprentice", normalizedEmail, 30 * 1000);
        ...
        apprenticeSignupInFlight = false;
        setButtonBusy(signupBtn, false);
    }
}
...
export async function getUserProfile(userId) {
    // Server-side trigger creates profile on signup; retry once if trigger is slightly delayed
    await refreshSubscriptionStatusIfPossible(userId);
    ...
    if (result.error && (result.error.code === "PGRST116" || result.error.message?.includes("not found"))) {
        await new Promise((resolve) => setTimeout(resolve, 1500));
    ...
    if (!result.data) {
        throw new Error("Profile not found. If you just signed up, try refreshing in a moment.");
    }
    return result.data;
}
...
export async function createJobRequest(userId, jobData) {
    try {
        ...
        // Check if wallet has sufficient funds (NGN-based)
        ...
        // Deduct escrow amount from wallet (NGN-driven, points derived)
        const newBalance = currentBalance - escrowAmount;
        const newBalancePoints = ngnToPoints(newBalance);

        console.log('[ESCROW HOLD] Client wallet update (NGN-driven):', {
            user_id: userId,
            old_balance_ngn: currentBalance,
            new_balance_ngn: newBalance,
            derived_old_points: ngnToPoints(currentBalance),
            derived_new_points: newBalancePoints
        });
        ...
        const transactionPoints = ngnToPoints(escrowAmount);
        const { error: transactionError } = await supabase
            ...
            .insert({
                ...
                amount_points: -transactionPoints,
    ...
    try {
        ...
        if (approved) {
            ...
            if (paymentAlreadyProcessed) {
                ...
                // If approved, mark as completed and release payment to apprentice (with 10% platform fee)
                updateData.status = "completed";

                const payout = calculatePayout(escrowAmount);
                const payment = payout.apprenticeAmount; // 90% to apprentice
        ...
        // Credit apprentice wallet (NGN-driven, points derived)
        const currentApprenticeBalance = apprenticeWallet.balance_ngn || 0;
        const newApprenticeBalance = currentApprenticeBalance + payment;
        const newApprenticePoints = ngnToPoints(newApprenticeBalance);

        console.log('[ESCROW RELEASE] Apprentice wallet update (NGN-driven):', {
            apprentice_id: job.assigned_apprentice_id,
            old_balance_ngn: currentApprenticeBalance,
            new_balance_ngn: newApprenticeBalance,
            derived_old_points: ngnToPoints(currentApprenticeBalance),
            derived_new_points: newApprenticePoints
        });
        ...
        const { error: apprenticeWalletError } = await supabase
                ...
                balance_points: newApprenticePoints,
                            updated_at: new Date().toISOString(),
                        })
        ...
        const apprenticePoints = ngnToPoints(payment);
        const { error: apprenticeTransactionError } = await supabase
                        ...
                        .insert({
                            ...
                            amount_ngn: payment,
                amount_points: apprenticePoints,
                    ...
                    // Record platform fee (10%) for audit
                    if (payout.fee > 0) {
                        const { error: platformFeeError } = await supabase
                            ...
                            .insert({
                                ...
                                amount_ngn: payout.fee,
                                amount_points: 0,
                                description: `Platform fee (10%) for job: ${job.title}`,
                                reference: `PLATFORM-FEE-${jobRequestId}`,
                                job_id: jobRequestId,
                                status: "completed",
                                metadata: {
                                    job_request_id: jobRequestId,
                                    job_title: job.title,
                                    original_amount: payout.originalAmount,
                                    apprentice_amount: payout.apprenticeAmount
                                }
                            });
                        if (platformFeeError) {
                            console.warn("Failed to record platform fee transaction:", platformFeeError);
                        }
                    }
            ...
            // Refund escrow to client wallet (NGN-driven, points derived)
            const currentClientBalance = clientWallet.balance_ngn || 0;
            const newClientBalance = currentClientBalance + escrowAmount;
            const newClientPoints = ngnToPoints(newClientBalance);

            console.log('[ESCROW REFUND] Client wallet update (NGN-driven):', {
                client_id: clientId,
                old_balance_ngn: currentClientBalance,
                new_balance_ngn: newClientBalance,
                derived_old_points: ngnToPoints(currentClientBalance),
                derived_new_points: newClientPoints
            });
            ...
            const { error: clientWalletError } = await supabase
                ...
                .update({
                    ...
                    balance_points: newClientPoints,
                    updated_at: new Date().toISOString(),
                })
                .eq("user_id", clientId);
            ...
            const clientPoints = ngnToPoints(escrowAmount);
...
async function processRefund(clientId, escrowAmount, jobRequestId, jobTitle) {
    ...
    try {
        ...
        // Refund escrow to client wallet (NGN-driven, points derived)
        const currentBalanceNgn = clientWallet.balance_ngn || 0;
        const newClientBalance = currentBalanceNgn + escrowAmount;
        const newClientPoints = ngnToPoints(newClientBalance);

        console.log('[REFUND] Wallet update (NGN-driven):', {
            client_id: clientId,
            old_balance_ngn: currentBalanceNgn,
            new_balance_ngn: newClientBalance,
            derived_old_points: ngnToPoints(currentBalanceNgn),
            derived_new_points: newClientPoints
        });
        ...
        const refundPoints = ngnToPoints(escrowAmount);
...
export async function createWithdrawalRequest(userId, pointsRequested, payoutMethod, payoutDetails) {
    try {
        ...
        // No withdrawal fee: user receives exactly what they request
        const amountNgn = pointsToNgn(pointsRequested);
        const totalDeductionPoints = pointsRequested;
        const totalDeductionNgn = amountNgn;
        const withdrawalFeePoints = 0;
        const withdrawalFeeNgn = 0;
        const availableNgn = wallet.balance_ngn || 0;

        if (availableNgn < amountNgn) {
            throw new Error(
                `Insufficient wallet balance. You requested ${pointsRequested} points (₦${amountNgn.toLocaleString()}). You have ₦${availableNgn.toLocaleString()} available.`
            );
        }
        ...
        // Reuse previously computed NGN values in direct-insert path (no fee)
        const derivedAmountNgn = amountNgn;
        const derivedWithdrawalFeeNgn = 0;
        const derivedTotalDeductionNgn = totalDeductionNgn;
        ...
        if (supportsExtendedWithdrawalColumns) {
            ...
            withdrawalInsertPayload.withdrawal_fee_ngn = derivedWithdrawalFeeNgn;
            withdrawalInsertPayload.total_deduction_points = totalDeductionPoints;
            withdrawalInsertPayload.total_deduction_ngn = derivedTotalDeductionNgn;
        }
...
export async function getAllWithdrawalRequests(status = null) {
    try {
        let query = supabase
            ...
            .select(`
                *,
                user:profiles!wallet_withdrawal_requests_user_id_fkey(
...
// Convert points to USD and NGN using shared NGN↔points rate
export function convertPointsToCurrency(points) {
    const ngn = pointsToNgn(points); // 1 point = POINT_TO_NGN_RATE NGN
    // Preserve USD helper semantics: derive USD from NGN using fixed rate to avoid breaking UI
    const usd = ngn / (POINT_TO_NGN_RATE * 10); // Backwards-compatible placeholder
    return { usd, ngn };
}
