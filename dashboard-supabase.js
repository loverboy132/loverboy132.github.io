...
import {
    ...
    // Subscription helpers: expiry-aware plan + status
    getSubscriptionActivity,
    getEffectiveSubscriptionPlanKey,
} from "./supabase-auth.js";
import { uploadCV, getCVSignedUrl, getUserWallet, createFundingRequest, getCraftnetBankAccounts, uploadProofOfPayment, calculatePoints } from "./manual-payment-system.js";
import {
        ...
        try {
            ...
            try {
                ...
                stats.availableBalance = Number(
                    referralWallet?.available_ngn ??
                        referralWallet?.available_points ??
                        0
                );
            } catch {
...
const memberContentTemplates = {
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-bold">${(
                            calculatePoints(userData.total_earnings || 0)
                        ).toLocaleString()}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ${searchResults
                    .map(
                        (user) => {
                            const yearsExp =
                                ...
                                null;
                            const yearsExpLine = yearsExp
                                ? `<p class="text-xs text-gray-500">Experience: ${yearsExp} yrs</p>`
                                : "";
                    ...
                    <div class="bg-white p-6 rounded-lg shadow hover:shadow-lg transition-shadow">
                        ...
                        <div class="flex justify-between items-center text-sm text-gray-500 mb-4">
                            ...
                            ${
                                ...
                                <span>${(
                                    ...
                                    calculatePoints(user.total_earnings || 0)
                        ...
                        <div class="flex space-x-2 explore-card-actions">
                            ${
                                currentUserRole === "member" &&
                                user.role === "apprentice"
                                    ? `
                            ...
                            <button class="flex-1 bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 ${
                                user.role === "apprentice" ? "more-btn" : "follow-btn"
                                    ...
                                    data-user-id="${user.id}"
                                    data-user-name="${(user.name || user.email || "User").replace(/"/g, "&quot;")}">
                                ${user.role === "apprentice" ? "More" : "Follow"}
                            </button>
                            `
                            }
                        </div>
                        ...
                        ? recommendations
                              .map(
                                  (user) => {
                                      ...
                                      const isConnectCase =
                                          currentUserRole === "member" && isApprentice;
                                      const primaryBtnClass = isConnectCase
                                          ? "more-btn"
                                          : "follow-btn";
                                      const primaryBtnText = isConnectCase
                                          ? "More"
                                          : "Follow";
    ...
    profile: (userData) => `
        ...
        <div class="bg-white p-8 rounded-lg shadow max-w-2xl mx-auto">
            ...
            <div class="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    <h4 class="font-semibold text-gray-800">Subscription</h4>
                    <p class="text-gray-600">
                        ${(() => {
                            const { isActive, expiresAt } = getSubscriptionActivity(userData);
                            const effectiveKey = getEffectiveSubscriptionPlanKey(userData);
                            const plan = subscriptionPlans[effectiveKey] || subscriptionPlans["free"];
                            ...
                            if (!isActive) {
                                const expiredOn = expiresAt
                                    ...
                                    : "in the past";
                                return `${plan.name} (Expired ${expiredOn})`;
                            }
                            const until = expiresAt ? expiresAt.toLocaleDateString() : "unknown date";
                            return `${plan.name} (Active until ${until})`;
                        })()}
                    </p>
                </div>
                <div class="bg-gray-50 p-4 rounded-lg">
    ...
    subscription: (userData) => `
        ...
        <div class="mb-6">
            ${(() => {
                const { isActive, expiresAt } = getSubscriptionActivity(userData);
                const rawPlan = (userData?.subscription_plan || "free").toLowerCase();
                if (rawPlan === "free") {
                    return '<p class="text-gray-600">You are currently on the Free plan.</p>';
                }
                if (!isActive) {
                    const expiredOn = expiresAt
                        ? expiresAt.toLocaleDateString()
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ${Object.entries(subscriptionPlans)
                ...
                <div class="bg-white p-6 rounded-lg shadow ${
                    ...
                    ${(() => {
                        const effectiveKey = getEffectiveSubscriptionPlanKey(userData);
                        const isCurrentEffective = effectiveKey === key;
                        if (isCurrentEffective) {
                            const { isActive } = getSubscriptionActivity(userData);
                            if (key === "free") {
    ...
    earnings: async (userData) => {
        ...
        try {
            ...
            withdrawalWindowOpen = currentDay >= 1 && currentDay <= 7;

            // Prefer server-side stats (counts + wallet), RLS-safe
            try {
                const { data, error } = await supabase.rpc("get_referral_stats");
                if (!error && data) {
                    referralSummary = Array.isArray(data) ? data[0] : data;
                }
            } catch (e) {
                // Ignore if RPC isn't deployed yet
            }
        } catch (error) {
        ...
        const subscriptionActivity = getSubscriptionActivity(userData || {});
        const isActiveMember = isMember && subscriptionActivity.isActive;
        const summary = referralSummary || {};
        const totalReferrals =
            typeof summary.total_referrals !== "undefined"
                ? Number(summary.total_referrals || 0)
                : Array.isArray(referralStats)
                      ? referralStats.length
                      : 0;
        const referredMembers =
            typeof summary.referred_members !== "undefined"
                ? Number(summary.referred_members || 0)
                : Array.isArray(referralStats)
                      ? referralStats.filter((r) => r.referred_role === "member")
                            .length
                      : 0;
        const referredApprentices =
            typeof summary.referred_apprentices !== "undefined"
                ? Number(summary.referred_apprentices || 0)
                : Array.isArray(referralStats)
                      ? referralStats.filter(
                            (r) => r.referred_role === "apprentice"
                        ).length
                      : 0;

        const availableNgn = Number(
            referralWallet?.available_ngn ??
                summary.available_ngn ??
                referralWallet?.available_points ??
                summary.available_points ??
                0
        );
        const lockedNgn = Number(
            referralWallet?.locked_ngn ??
                summary.locked_ngn ??
                referralWallet?.locked_points ??
                summary.locked_points ??
                0
        );
        const totalNgn = Number(
            referralWallet?.total_ngn ??
                summary.total_ngn ??
                referralWallet?.total_points ??
                summary.total_points ??
                0
        );

        const inviteLink =
            isMember && referralCode && referralCode.code
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-600">Earn NGN bonuses from referrals and manage your wallet</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                <h3 class="text-xl font-bold mb-4">${isMember ? "Your Referral Link" : "Referrals"}</h3>
                <div class="space-y-4">
                    ${
                        !isMember
                            ? `
                    <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                        <p class="text-gray-800 font-medium">Referrals are exclusive to Members.</p>
                    </div>
                    `
                            : `
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                        ...
                        ${
                            ...
                            <div class="flex items-center justify-between text-sm text-gray-600">
                                ...
                                <span>Total Referrals: <span class="font-semibold">${totalReferrals}</span></span>
                            </div>
                            <div class="mt-2 flex items-center justify-between text-sm text-gray-600">
                                <span>Members: <span class="font-semibold">${referredMembers}</span></span>
                                <span>Apprentices: <span class="font-semibold">${referredApprentices}</span></span>
                            </div>
                        `
                                : `
                            <div class="text-center py-4">
                                <p class="text-gray-700 font-medium">${
                                    isMember
                                        ...
                                        : "Referrals are exclusive to Members."
                                }</p>
                                <p class="text-gray-600 text-sm mt-1">${
                                    isMember
                                        ? "Please refresh the page or try again later."
                                        : "You can still be referred, but you won’t have a referral code."
                                }</p>
                                ${
                                    isMember
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <div>
                                                <p class="font-medium text-sm">${
                                                    earning.event_type === 'apprentice_job'
                                                        ? '+₦50 — Referred apprentice completed a job'
                                                        : '+₦150 — New member subscribed'
                                                }</p>
            ...
            <!-- Withdrawal Section -->
            <div class="bg-white rounded-lg shadow-md p-6">
                <h3 class="text-xl font-bold mb-4">Withdraw Referral Earnings</h3>
                <div class="space-y-4">
                    ${
                        !isMember
                            ? `
                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                            <p class="text-gray-800 font-medium">Only Members can earn and withdraw referral bonuses.</p>
                        </div>
                    `
                            : !isActiveMember
                            ? `
                    ...
                    `
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Amount to Withdraw (NGN)</label>
                                <input 
                                    type="number" 
                                    id="withdrawal-amount-ngn" 
                                    min="1000" 
                                    max="${availableNgn || 0}"
                        ...
                        <div class="border-t border-gray-200 pt-4">
                            ...
                            <div class="space-y-2">
                                ${withdrawalRequests
                                    ...
                                    <div class="flex items-center justify-between p-2 bg-gray-50 rounded">
                                        <div>
                                            <p class="font-medium text-sm">₦${Number(request.amount ?? request.amount_ngn ?? 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}</p>
    ...
    jobs: async (userData) => {
        ...
        // Count monthly jobs for free/expired plan users
        let monthlyJobCount = 0;
        const subscriptionPlan = getEffectiveSubscriptionPlanKey(userData);
    ...
    if (userData.role === "member") {
        ...
        if (tabId === "explore") {
            ...
            async function handleSearch() {
                ...
                await displaySearchResults(results, userData.role);
...
document.addEventListener("click", async (e) => {
    // "More" button handler (Explore/Recommended/Search cards)
    const moreBtn = e.target.closest(".more-btn");
    if (moreBtn) {
        e.preventDefault();
        e.stopPropagation();

        const userId = moreBtn.dataset.userId;
        const userName = moreBtn.dataset.userName || "User";
        if (!userId) return;

        await showUserProfileModal(userId, userName);
        return;
    }

    // View profile button (Explore search results: member viewing apprentices)
    const viewProfileBtn = e.target.closest(".view-profile-btn");
    if (viewProfileBtn) {
        e.preventDefault();
        e.stopPropagation();
        const targetUserId = viewProfileBtn.dataset.userId;
        const targetUserName = viewProfileBtn.dataset.userName || "User";
        await showUserProfileModal(targetUserId, targetUserName);
        return;
    }

    // Personal Hire button (Explore search results: member viewing apprentices)
    const personalHireBtn = e.target.closest(".personal-hire-btn");
    if (personalHireBtn) {
        e.preventDefault();
        e.stopPropagation();
        const apprenticeId = personalHireBtn.dataset.userId;
        const apprenticeName = personalHireBtn.dataset.userName || "Apprentice";
...
async function displaySearchResults(results, currentUserRole) {
    const searchResultsDiv = document.getElementById("search-results");
    if (!searchResultsDiv) return;

    // Dedupe by user.id to guarantee unique search results
    ...
    // Always clear container first (never append on top of old results)
    searchResultsDiv.innerHTML = "";

    const isMemberViewingApprentices = currentUserRole === "member";
    ...
    if (uniqueResults.length === 0) {
        ...
        searchResultsDiv.innerHTML = `
            <h3 class="text-xl font-bold text-gray-900 mb-4">Search Results (${uniqueResults.length})</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ${resultsWithRatings
                    .map(
                        (user) => {
                ...
                .map(
                    (user, index) => {
                        ...
                        const yearsExpLine = yearsExp
                            ...
                            : "";
                        const isConnectCase =
                            userData.role === "member" && user.role === "apprentice";
                        const primaryBtnClass = isConnectCase ? "more-btn" : "follow-btn";
                        const primaryBtnText = isConnectCase ? "More" : "Follow";
...
async function handleWorkUpload(e, userData) {
    ...
    try {
        ...
        // 4. Points are derived from money (total_earnings) - no manual point awards
...
function getNotificationEls() {
    return {
        ...
        markAllBtn: document.getElementById("mark-all-notifications"),
    };
}

function bindNotificationUIHandlers() {
    const { bell, panel } = getNotificationEls();
    if (!bell) return;
    ...
    bell.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        // Temporary: remove after confirming bell → panel works
...
async function initializeNotificationCenter(userData) {
    const { bell, list } = getNotificationEls();
    if (!bell || !list) return;

    // Always ensure click handlers exist (even after header rebuild)
    bindNotificationUIHandlers();

    // Only do data setup once
    ...
    const { markAllBtn } = getNotificationEls();
    if (markAllBtn) {
        markAllBtn.addEventListener("click", async () => {
            ...
            try {
                ...
                console.error("markAll notifications failed:", err);
                await refreshNotifications();
            }
        });
    }
...
function renderNotificationList() {
    ...
    list.innerHTML = notificationState.items
        .map((notification) => {
...
function toggleNotificationPanel(forceState) {
    ...
    if (shouldOpen) {
        ...
        // mobile positioning
        if (window.innerWidth <= 640 && bell && wrapper) {
            const bellRect = bell.getBoundingClientRect();
            const viewportWidth = window.innerWidth;
            const panelWidth = Math.min(380, viewportWidth * 0.92);

            panel.style.position = "fixed";
            panel.style.right = `${viewportWidth - bellRect.right}px`;
...
// Global fallback: bell click works even if header was rebuilt and direct handlers were lost
document.addEventListener("click", (e) => {
    if (e.target.closest && e.target.closest("#notification-bell")) {
        e.preventDefault();
        e.stopPropagation();
        toggleNotificationPanel();
    }
...
async function renderDashboard(user, userData) {
    try {
        ...
        // Normalize dashboard header layout for mobile
        normalizeDashboardHeaderLayout();
        if (typeof bindNotificationUIHandlers === "function") {
            bindNotificationUIHandlers();
        }
        ...
        if (userData.role === "apprentice") {
            ...
            const { isActive, expiresAt } = getSubscriptionActivity(userData);
            const effectivePlanKey = getEffectiveSubscriptionPlanKey(userData);
            const planConfig = subscriptionPlans[effectivePlanKey] || subscriptionPlans["free"];

            if (userRoleInfoEl) {
                let label = `${planConfig.name} Plan`;
                if (!isActive && effectivePlanKey !== "free") {
                    label += " (Expired)";
                }
                userRoleInfoEl.textContent = label;
            }
...
// Helper function to normalize dashboard header layout for mobile
function normalizeDashboardHeaderLayout() {
  const dashboardHeader = document.querySelector('#dashboard-layout header');
  if (!dashboardHeader) return;

  // Prevent re-running
  if (dashboardHeader.querySelector('.dashboard-header-row')) return;
  ...
  const logoSvg = dashboardHeader.querySelector('svg');
  const logoImg = dashboardHeader.querySelector('img:not(#user-avatar)');

  // Right-side items (prefer wrapper so panel stays with bell)
  const notificationWrapperEl = document.getElementById('notification-wrapper');
  ...
  leftGroup.className = 'dashboard-left';
  ...
  if (logoLink && (logoLink.querySelector('svg') || logoLink.querySelector('img'))) {
    ...
    leftGroup.appendChild(logoSvg);
  }

  if (notificationWrapperEl) {
    rightGroup.appendChild(notificationWrapperEl);
  } else if (notificationBellEl) {
    rightGroup.appendChild(notificationBellEl);
  }
  if (userAvatar) rightGroup.appendChild(userAvatar);
  if (logoutButton) rightGroup.appendChild(logoutButton);

  headerRow.appendChild(leftGroup);
  headerRow.appendChild(rightGroup);

  // ✅ Do NOT nuke the whole header. Remove only non-nav children safely.
  // We'll rebuild header as: [row] + [mainNav (if exists)]
  const preservedNav = mainNav ? mainNav : null;
...
async function showUserProfileModal(userId, userName) {
    try {
        ...
        // Optional CV link (modal only). Fail gracefully if field doesn't exist.
        const cvUrl =
            userProfile?.cv_url ??
            userProfile?.cvUrl ??
            userProfile?.cv ??
...
function renderFinalSubmissionFile(container, publicUrl, filePath) {
    ...
    // Add a separate download action without altering preview rendering.
    // Important: use the SAME URL already used for preview (often a signed URL).
    const existingLink = container.querySelector('a[href]');
    if (existingLink) {
        // For non-previewable types, the preview is already a link; make it download-capable.
        existingLink.setAttribute("download", "");
        existingLink.setAttribute("rel", "noopener noreferrer");
        existingLink.classList.add("download-final-btn");
        existingLink.addEventListener("click", (e) => e.stopPropagation());
    } else {
        // For previewable types (image/pdf/video), append a small download button below.
        const downloadRow = document.createElement("div");
        downloadRow.className = "final-download-row";
        ...
        downloadLink.className = "download-final-btn";
...
async function handleWithdrawalRequest() {
    try {
        const amountInput = document.getElementById('withdrawal-amount-ngn');
        ...
        if (currentDay < 1 || currentDay > 7) {
            showNotification('Withdrawals are only processed between the 1st and 7th of each month', 'error');
            return;
        }
        ...
        showNotification(
            'Your withdrawal request has been submitted. Admin will process it within 24 hours.',
            'success'
        );
