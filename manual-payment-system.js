...
import { POINT_TO_NGN_RATE, ngnToPoints, pointsToNgn } from "./supabase-client.js";
import { calculatePoints, calculatePayout, POINT_CONVERSION_RATE, PLATFORM_FEE_RATE } from "./payout-constants.js";

// Re-export for consumers
export { calculatePoints, calculatePayout, POINT_CONVERSION_RATE, PLATFORM_FEE_RATE };
import { 
...
export async function updateWalletBalanceAtomically(userId, balanceChanges) {
    const {
        balance_ngn = 0,
        // balance_points is intentionally ignored to keep NGN as the single source of truth
        balance_points = 0,
    ...
    try {
        console.log('Starting atomic wallet update:', {
            userId,
            balance_ngn_delta: balance_ngn,
            balance_points_delta_ignored: balance_points,
            total_deposited_delta: total_deposited,
            total_withdrawn_delta: total_withdrawn,
        ...
        const currentBalanceNgn = currentWallet.balance_ngn || 0;
        const currentBalancePointsDerived = ngnToPoints(currentBalanceNgn);

        console.log('Current wallet state (NGN as source of truth):', {
            balance_ngn: currentBalanceNgn,
            derived_balance_points: currentBalancePointsDerived,
            stored_balance_points: currentWallet.balance_points || 0,
            total_deposited: currentWallet.total_deposited || 0,
            total_withdrawn: currentWallet.total_withdrawn || 0
        });

        // Calculate new balances
        const newBalanceNgn = currentBalanceNgn + balance_ngn;
        const newBalancePoints = ngnToPoints(newBalanceNgn);
        ...
        console.log('Calculated new balances (NGN-driven):', {
            new_balance_ngn: newBalanceNgn,
            new_balance_points_derived: newBalancePoints,
            new_total_deposited: newTotalDeposited,
            new_total_withdrawn: newTotalWithdrawn
        });

        // Validate balances (prevent negative NGN balances)
        if (newBalanceNgn < 0) {
        ...
        console.log('Wallet update completed successfully (NGN-driven):', {
            old_balance_ngn: currentBalanceNgn,
            old_derived_balance_points: currentBalancePointsDerived,
            new_balance_ngn: updatedWallet.balance_ngn,
            new_derived_balance_points: ngnToPoints(updatedWallet.balance_ngn),
...
export async function createWithdrawalRequest(amountPoints, bankDetails) {
    try {
        ...
        const totalDeductionPoints = amountPoints;
        ...
        if (availableNgn < amountNgn) {
            throw new Error(
                `Insufficient wallet balance. You requested ${amountPoints} points (₦${amountNgn.toLocaleString()}). You have ₦${availableNgn.toLocaleString()} available.`
            );
        }
        ...
        // Insert directly into table, reusing previously computed NGN values
        const derivedAmountNgn = amountNgn;
        const derivedWithdrawalFeeNgn = withdrawalFeeNgn;
        ...
        const extendedWithdrawalPayload = {
            ...
            withdrawal_fee_ngn: derivedWithdrawalFeeNgn,
            total_deduction_points: totalDeductionPoints,
            total_deduction_ngn: derivedTotalDeductionNgn
        };
...
export async function approveFundingRequest(requestId, accountDetails = null) {
    try {
        ...
        console.log('Current wallet balance before credit (NGN-driven):', {
            balance_ngn: wallet.balance_ngn,
            derived_balance_points: ngnToPoints(wallet.balance_ngn || 0),
            stored_balance_points: wallet.balance_points,
            user_id: userId,
            wallet_id: wallet.id
        });
            ...
            console.log('Attempting to credit wallet (NGN-driven):', {
                user_id: userId,
                wallet_id: wallet.id,
            ...
            const expectedDerivedPoints = ngnToPoints(newBalanceNgn);
            const actualDerivedPoints = ngnToPoints(updatedWallet.balance_ngn || 0);
            const pointsDiff = Math.abs(actualDerivedPoints - expectedDerivedPoints);
            ...
            if (ngnDiff > 0.01 || pointsDiff > 0.01) {
                console.error('Wallet balance mismatch:', {
                    ...
                    expected_derived_points: expectedDerivedPoints,
                    actual_derived_points: actualDerivedPoints,
            ...
            console.log('Wallet credited successfully (NGN-driven):', {
                previous_balance_ngn: wallet.balance_ngn,
                previous_derived_balance_points: ngnToPoints(wallet.balance_ngn || 0),
                amount_added_ngn: amountToCredit,
                points_added: pointsToAdd,
                new_balance_ngn: updatedWallet.balance_ngn,
                new_derived_balance_points: ngnToPoints(updatedWallet.balance_ngn || 0)
            });
            ...
            const { data: transactionData, error: transactionError } = await supabase
                ...
                .insert({
                    ...
                    amount_ngn: amountToCredit,
                    amount_points: ngnToPoints(amountToCredit),
...
export async function approveWithdrawalRequest(requestId, bankTransactionId = null) {
    try {
        ...
        // No withdrawal fee: deduct exactly the requested amount
        const feePoints = 0;
        const feeNgn = 0;
        const pointsToDeduct = withdrawalRequest.amount_points;
        const ngnToDeduct = withdrawalRequest.amount_ngn;
        ...
        // Check if user has enough balance (including fees) - NGN-based check
        const availableNgn = wallet.balance_ngn || 0;
        if (availableNgn < ngnToDeduct) {
            throw new Error(`User does not have sufficient wallet balance. Required: ₦${ngnToDeduct.toLocaleString()}, Available: ₦${availableNgn.toLocaleString()}`);
        }
        ...
        try {
            ...
            if (data && !error) {
                ...
                // Verify that RPC actually deducted balance by checking wallet balance
                const updatedWallet = await getUserWallet(withdrawalRequest.user_id);
                if (updatedWallet && (updatedWallet.balance_ngn || 0) >= (wallet.balance_ngn || 0)) {
                    // RPC didn't deduct NGN, we need to do it manually
                    console.warn('RPC approved but did not deduct NGN balance, deducting manually...');
                    rpcSucceeded = false;
                } else if (updatedWallet) {
                    console.log(`RPC successfully deducted NGN: ₦${((wallet.balance_ngn || 0) - (updatedWallet.balance_ngn || 0)).toLocaleString()}`);
                }
            }
        } catch (rpcError) {
            // If RPC function doesn't exist or fails, we'll handle it manually
        ...
        if (!existingTransaction || checkError?.code === 'PGRST116') {
            ...
            const currentBalanceNgn = currentWallet.balance_ngn || 0;
            const currentDerivedPoints = ngnToPoints(currentBalanceNgn);
            const newBalanceNgn = currentBalanceNgn - ngnToDeduct;
            const newDerivedPoints = ngnToPoints(newBalanceNgn);

            console.log(`[Withdrawal Approval] Current balance before deduction (NGN-driven):`, {
                balance_ngn: currentBalanceNgn,
                derived_balance_points: currentDerivedPoints
            });
            console.log(`[Withdrawal Approval] Deducting (NGN-driven):`, {
                ngn_to_deduct: ngnToDeduct,
                derived_points_to_deduct: ngnToPoints(ngnToDeduct)
            });

            // Verify user still has sufficient balance (NGN)
            if (currentBalanceNgn < ngnToDeduct) {
                throw new Error(`User wallet balance insufficient. Required: ₦${ngnToDeduct.toLocaleString()}, Available: ₦${currentBalanceNgn.toLocaleString()}`);
            }

            // Deduct NGN from wallet using atomic update function (points are derived)
            ...
            console.log('[Withdrawal Approval] Wallet updated (NGN-driven):', {
                old_balance_ngn: currentBalanceNgn,
                new_balance_ngn: updatedWallet.balance_ngn,
                old_derived_balance_points: currentDerivedPoints,
                new_derived_balance_points: ngnToPoints(updatedWallet.balance_ngn || 0)
            });
            ...
            const transactionData = {
                ...
                amount_points: -ngnToPoints(ngnToDeduct),
...
async function processSubscriptionPaymentFallback(user, planKey, amountNgn, paymentMethod) {
    ...
    // Activate timed subscription server-side (1 month from now)
    try {
        const { error: activateError } = await supabase.rpc('activate_subscription', {
            p_user_id: user.id
        });
        if (activateError) {
            console.error('Failed to activate subscription via RPC:', activateError);
            throw new Error(activateError.message || "Failed to activate subscription.");
        }
    } catch (e) {
        // Fallback: at least set the plan, even if expiry RPC is missing
        const msg = e?.message || "";
        if (e?.code === '42883' || msg.includes('does not exist')) {
            console.warn('activate_subscription RPC missing, falling back to direct plan update');
...
async function updateWalletBalanceForSubscription(userId, newBalance, totalWithdrawn) {
    const derivedPoints = ngnToPoints(newBalance);

    console.log('[Subscription Wallet Update] Updating wallet (NGN-driven):', {
        user_id: userId,
        new_balance_ngn: newBalance,
        new_derived_balance_points: derivedPoints,
        new_total_withdrawn: totalWithdrawn
    });
...
async function logSubscriptionWalletTransaction(userId, amount, planKey, paymentMethod, reference) {
    try {
        const amountPoints = ngnToPoints(amount);
...
export async function approveSubscriptionPayment(requestId, adminNotes = null) {
    try {
        ...
        if (!rpcSucceeded) {
            ...
            // Activate the user's subscription (timed, server-side)
            console.log('Activating user subscription via RPC...', {
            ...
            try {
                ...
                if (e?.code === '42883' || msg.includes('does not exist')) {
                    ...
                    if (profileError) {
                        ...
                        console.log('User subscription plan updated successfully (fallback):', updatedProfile);
                        profileUpdated = true;
                    } else {
                        console.warn('Profile update returned no rows - user may not exist or RLS blocked the update');
                    }
                } else {
                    console.error('Unexpected error activating subscription:', e);
                }
            }
